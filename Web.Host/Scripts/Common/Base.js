    ...
    $.fn.CheckBoxSelect3 = function (attrSelector) {
        if ($(this).get(0).checked) {
            $("input[type=checkbox][" + attrSelector + "]").each(function () {
                if ($(this).attr("disabled") != "disabled") {//非禁用状态才执行
                    this.checked = true;
                    $(this).change();
                }
            });
   ...
   */
    ...
    */
    $.fn.CheckServerRedirect = function(data) {
        if (typeof data === "string" && data.indexOf('<meta name="Portal-LoginPage">') > -1) {
        ...
        var sendOption = {
            ...
            success: function (data) {
                if (!$.fn.CheckServerRedirect(data)) {
                    ...
                    //格式化utc时为当地时
                    $("[utc]").each(function () {
                        var notAjaxUtc = $(this).attr("notAjaxUtc");
                        if (typeof (notAjaxUtc) == "undefined") {//标记了notAjaxUtc的时间，ajax success不更新utc时
    ...
    */
    $.FormSubmit = function (formname, url, outputConfig, target, method) {
        ...
        if (download.length < 1) {
            $("body").append("<form id='" + formname + "' action='' method='" + method + "' target='" + target + "' style='display:none;'></form>");
            download = $("#" + formname);
        } else {
            download.empty();
        }
