...
*/
    ...
    c$.InitMsg = function (result, obj, errMessage) {
        var element = obj.get(0);
        var nextElement = element.nextSibling;
        if (!isUndefinedOrNull(nextElement) && nextElement.element === element) {
            $(nextElement).popover('destroy');
        ...
        if (result) {
            ...
            c$.appendFont(element, errMessage);
        }
    };
    c$.appendError = function (element, errMessage) {
        $(element).addClass("error-input");
        c$.appendFont(element, errMessage);
    };
    c$.appendFont = function (element, errMessage) {
        var font2 = document.createElement('font');
        font2.element = element;
        font2.className = "error-icon";
        font2.title = "错误提示";
        var fontObj = $(font2);
        fontObj.attr("data-container", "body");
        fontObj.attr("data-toggle", "popover");
        fontObj.attr("data-placement", "right");
        fontObj.attr("data-content", errMessage);
        // 插入到元素的后面
        var pos = element.nextSibling;
        if (isUndefinedOrNull(pos)) {
            element.parentNode.appendChild(font2, element);
        } else {
            pos.parentNode.insertBefore(font2, pos);
        }
        fontObj.popover({ trigger: 'hover' });
    };
    /**
    * 检查一个元素
    ...
    c$.checkElement2 = function (element, type) {
        ...
        */
        var showDialog = thisFun.showDialog = thisFun.showDialog || function (key, element, message, C_pos, S_pos, iniMess) {
            ...
            // "S"类型用文字显示到页面
            if (S_pos) {
                c$.appendError(element, alert_message);
            }
            else {
                // 使用 alert 提示出来
